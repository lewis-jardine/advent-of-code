...
def test_shortend_reports(report: list[int]) -> bool:
    """Test if a report is safe when removing any letter"""
    for n in range(len(report)):
        copy_report = report.copy()
        del copy_report[n]
        if report_safe(copy_report):
            return True
    return False
...
def main():
    ...
    for line in lines:
        ...
        if test_shortend_reports(report):
            safe_reports += 1
